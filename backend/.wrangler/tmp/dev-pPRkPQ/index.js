...
// .wrangler/tmp/bundle-WqCqOk/checked-fetch.js
...
// .wrangler/tmp/bundle-WqCqOk/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-WqCqOk/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
async function signin(c) {
  ...
  try {
    ...
    const jwt3 = await sign2({ id: userexists.id }, c.env.SECRET_KEY);
    return c.text(jwt3);
  } catch (e) {
    c.status(411);
    return c.text("Invalid request");
...
`;
    console.log("Blog table exists:", result);
...
init_checked_fetch();
init_modules_watch_stub();
async function authfunc(c, next) {
  try {
